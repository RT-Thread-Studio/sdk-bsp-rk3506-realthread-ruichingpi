...
{
   ec_master_t *master;
   void (*handler)(struct ec_master *master, uint16_t slave, uint8_t *input, uint8_t *output);
};

struct ecat_error_handler_arg
{
   ec_master_t *master;
   void (*handler)(struct ec_master *master, uint32_t error_code, const unsigned char *error_str);
};
