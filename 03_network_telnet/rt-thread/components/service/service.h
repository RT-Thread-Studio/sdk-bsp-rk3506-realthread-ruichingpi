...
/* service cmd */
#define SERVICE_CMD_GET_STATUS      0x01  /* 获取服务状态 */
#define SERVICE_CMD_SET_MODE        0x02  /* 设置服务模式 */
#define SERVICE_CMD_RESET           0x03  /* 重置服务    */
...
struct service_core_ops
{
    ...
    rt_ssize_t (*read)(struct service_core *service, rt_off_t pos, void *buffer,
        rt_size_t size);
    rt_ssize_t (*write)(struct service_core *service, rt_off_t pos,
...
typedef rt_err_t (*__kpi_service_register)(struct service_core *service,
    const char *name, rt_uint16_t flags, void *user_data);
...
typedef rt_err_t (*__kpi_service_open)(struct service_core *service,
    rt_uint16_t oflag);
typedef rt_err_t (*__kpi_service_close)(struct service_core *service);
typedef rt_ssize_t (*__kpi_service_read)(struct service_core *service,
...
typedef rt_err_t (*__kpi_service_control)(struct service_core *service, int cmd,
    void *arg);
