...
#if defined(COMPONENTS_WEBCLIENT_USING_MBED_TLS) || defined(COMPONENTS_WEBCLIENT_USING_SAL_TLS)
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    {
#if defined(COMPONENTS_WEBCLIENT_USING_SAL_TLS)
        session->is_tls = RT_TRUE;
#elif defined(COMPONENTS_WEBCLIENT_USING_MBED_TLS)
    ...
    {
#ifdef COMPONENTS_WEBCLIENT_USING_SAL_TLS
        if (session->is_tls)
        {
...
int webclient_read(struct webclient_session *session, void *buffer, size_t length)
{
    ...
    do
    {
        ...
        if (bytes_read <= 0)
        {
#if defined(COMPONENTS_WEBCLIENT_USING_SAL_TLS) || defined(COMPONENTS_WEBCLIENT_USING_MBED_TLS)
            if(session->is_tls &&
