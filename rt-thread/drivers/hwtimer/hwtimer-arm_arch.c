...
rt_used static rt_err_t arm_arch_timer_local_disable(void)
{
    rt_err_t ret = RT_EOK;
...
rt_used static rt_err_t arm_arch_timer_set_frequency(rt_uint64_t frq)
{
    rt_err_t ret = RT_EOK;
...
rt_used static rt_uint64_t arm_arch_timer_get_frequency(void)
{
    rt_uint64_t frq;
...
rt_used static rt_err_t arm_arch_timer_set_value(rt_uint64_t val)
{
    rt_err_t ret = RT_EOK;
...
rt_used static rt_uint64_t arm_arch_timer_get_value(void)
{
    rt_uint64_t val = 0;
...
static rt_err_t arm_arch_timer_probe(struct rt_platform_device *pdev)
{
    ...
    arm_arch_timer_irq =
        rt_dm_dev_get_irq_by_name(&pdev->parent, irq_name[irq_idx]);
    ...
    rt_hw_interrupt_install(
        arm_arch_timer_irq, arm_arch_timer_isr, RT_NULL, "tick-arm-timer");
