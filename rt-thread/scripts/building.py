...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = [], app = False):
    ...
    if build_type:
        if build_type in ['debug', 'release']:
            ...
            sys.stderr.write("Invalid option for --build-type: {}. Must be 'debug' or 'release'.\n".format(build_type))
            sys.exit(1)
    ...
    env['LINKCOM'] = '$LINK -o build/$TARGET $LINKFLAGS $__RPATH $SOURCES $_LIBDIRFLAGS -Wl,--start-group $_LIBFLAGS -Wl,--end-group'
    ...
    contents = f.read()
    f.close()
    f = open(os.path.join(RTT_ROOT, 'include', 'kconfig', rtconfig.CHIP, 'kconfig.h'), 'r')
    contents += f.read()
    f.close()
    ...
    if GetOption('pyconfig-silent'):
        from menu import guiconfig_silent
        guiconfig_silent(Rtt_Root)
        exit(0)

    elif GetOption('pyconfig'):
        from menu import guiconfig
        guiconfig(Rtt_Root)
        exit(0)

    configfn = GetOption('useconfig')
    if configfn:
        from menu import mk_rtconfig
    ...
    if not GetOption('verbose'):
        ...
        env.Replace(
            ...
            DTCCOMSTR  = '  DTC  $TARGET',
            LDCOMSTR = '  LD   $TARGET'
        )

    if GetOption('app_pkg'):
        from app_pkg import pack_bin
        debug_mode = GetOption('debug_mode') or 0
        pack_bin(GetOption('app_pkg'), debug_mode)
        exit(0)
    ...
    if not app:
        ...
        # include examples
        if os.path.isfile(os.path.join(Rtt_Root, 'examples/SConscript')):
            objs.extend(SConscript(os.path.join(Rtt_Root, 'examples/SConscript'),
                            variant_dir=kernel_vdir + '/examples',
                            duplicate=0))

        # include testcases
        if os.path.isfile(os.path.join(Rtt_Root, 'testcases/SConscript')):
            objs.extend(SConscript(os.path.join(Rtt_Root, 'testcases/SConscript'),
                            variant_dir=kernel_vdir + '/testcases',
                            duplicate=0))
...
def PrepareModuleBuilding(env, root_directory, bsp_directory):
    ...
    contents = f.read()
    f.close()
    f = open(bsp_directory + '/kconfig.h', 'r')
    contents += f.read()
    f.close()
    PreProcessor.process_contents(contents)
    BuildOptions = PreProcessor.cpp_namespace
...
def PrepareDTC(src, depend, **parameters):
    global Dtc
    ...
    if 'DTCFLAGS' in Dtc:
        target = Dtc['DTCFLAGS']
        if len(target) > 0:
            Env.AppendUnique(DTCFLAGS = target)
    ...
    if 'DTCPATH' in Dtc:
        ...
        if _PretreatListParameters(target) == True:
            ...
            target = paths
            Env.AppendUnique(DTCPATH = target)
...
def DefineGroup(name, src, depend, **parameters):
    global Env
    if not GetDepend(depend):
        if GetOption('gen-sdk-config'):
            add_unused_group(name, Rtt_Root, GetCurrentDir())
        return []
    ...
    if group['name'] != 'Kernel':
        ...
        for root, dirs, files in os.walk(cwd):
            for file in files:
                if file.endswith(".o"):
                    o_file = os.path.join(root, file)
                    o_files.append(o_file)
                    for s in group['src']:
                        s_no_suffix, ext1 = os.path.splitext(str(s))
                        o_file_no_suffix, ext2 = os.path.splitext(o_file)

                        if s_no_suffix == o_file_no_suffix:
                            group['src'].remove(s)

        for o_file in o_files:
            group['src'] += Glob(o_file)
...
def DoBuilding(target, objects):
    ...
    def dt_cmd(target, source, env):
        name = os.path.basename(source)
        path = os.path.dirname(os.path.join('build', os.path.splitext(source)[0]))
        if not os.path.exists(path):
            os.makedirs(path)

        dtb_d_pre_tmp = os.path.join(path, '%s.dtb.d.pre.tmp' % name)
        dtb_dts_tmp = os.path.join(path, '%s.dtb.dts.tmp' % name)
        dtb_d_dtc_tmp = os.path.join(path, '%s.dtb.d.dtc.tmp' % name)

        cpp_cmd = '%s -E -Wp,-MMD,%s -nostdinc ' % (env['CPP'], dtb_d_pre_tmp)
        for dtcpath in env['DTCPATH']:
            cpp_cmd += '-I%s ' % (dtcpath)
        cpp_cmd += '-undef -D__DTS__ -x assembler-with-cpp -o %s %s' % (dtb_dts_tmp, source)
        dtc_cmd = '%s -o build/%s %s -d %s %s' % (env['DTC'], target, env['DTCFLAGS'],
            dtb_d_dtc_tmp, dtb_dts_tmp)

        return [cpp_cmd, dtc_cmd]

    def dt_action(target, source, env):
        import subprocess

        cmd = dt_cmd(str(target[0]), str(source[0]), env)
        p = SCons.Action._subproc(env, cmd[0], stdin='devnull', shell=True)
        status = p.wait()
        if status:
            return status
        p = SCons.Action._subproc(env, cmd[1], stdin='devnull', shell=True)
        status = p.wait()
        if status:
            return status
    ...
    def dt_string(target, source, env):
        cmd = dt_cmd(str(target[0]), str(source[0]), env)
        return cmd[0] + '\n' + cmd[1]

    dt_scanner = SCons.Scanner.ClassicCPP("CScanner", ".dts", "DTCPATH",
        '^[ \t]*#[ \t]*(?:include|import)[ \t]*(<|")([^>"]+)(>|")')

    if 'DTCCOMSTR' in Env and Env['DTCCOMSTR']:
        dt_action = Action(dt_action, '$DTCCOMSTR')
    else:
        dt_action = Action(dt_action, dt_string)
    dt_builder = SCons.Builder.Builder(action = dt_action,
                                       src_suffix = '.dts',
                                       suffix = '.dtb',
                                       source_scanner = dt_scanner,
                                       single_source=True)
    Env.Append(BUILDERS = {'DTC': dt_builder})

    global Dtc
    if Dtc != {}:
        # Env.DTC(target.split('.')[0] + '.dtb', Dtc['src'])
        Env.DTC('board.dtb', Dtc['src'])
    ...
    if lib_name:
        ...
        for group in Projects:
            ...
            if 'LOCAL_CFLAGS' in group or 'LOCAL_CXXFLAGS' in group or 'LOCAL_CCFLAGS' in group or 'LOCAL_CPPPATH' in group or 'LOCAL_CPPDEFINES' in group:
                CFLAGS = CFLAGS + group.get('LOCAL_CFLAGS', '')
                CCFLAGS = CCFLAGS + group.get('LOCAL_CCFLAGS', '')
                CXXFLAGS = CXXFLAGS + group.get('LOCAL_CXXFLAGS', '')
                CPPPATH = CPPPATH + group.get('LOCAL_CPPPATH', [''])
                CPPDEFINES = list(CPPDEFINES) + group.get('LOCAL_CPPDEFINES', [''])
                ASFLAGS = ASFLAGS + group.get('LOCAL_ASFLAGS', '')
            ...
            for source in group['src']:
                ...
                if file_ext == '.o':
                    if source not in objects:
                        objects.append(source)
                    continue
                group_objects.append(Env.Object(source, CFLAGS = CFLAGS, CCFLAGS = CCFLAGS,
                CXXFLAGS = CXXFLAGS, ASFLAGS = ASFLAGS,
                CPPPATH = CPPPATH, CPPDEFINES = CPPDEFINES))

            group_objects = sorted(group_objects)

            if group_objects:
                name = File(os.path.join(os.getcwd(), 'build', '%s-builtin.o' % group['name']))
                big_object= Env.Command(target=name, source=group_objects, action=Action('$LD -r -o $TARGET $SOURCES', '$LDCOMSTR'))
                objects.append(big_object)
...
def GenTargetProject(program = None):
    ...
    if GetOption('target') == 'cic':
        from cic import GenerateCIC
        GenerateCIC(Env, Rtt_Root, str(Dir('#')))
...
def EndBuilding(target, program = None):
    ...
    Clean(target, 'build/app.bin')
    Clean(target, 'build/app.elf')
    Clean(target, 'build/app.map')
    Clean(target, 'build/app.img')
    ...
    config_name = GetOption('gen-sdk-config')
    if config_name:
        add_compiled_group(Projects, Rtt_Root)
        save_sdk_info(config_name, Rtt_Root)
    ...
    if config_name:
        print(target)
        action = SCons.Action.Action(gen_sdk, '\nGen %s SDK....' % project_name)
        y = Env.Command(target='nothing', source=None, action=action)
        Env.Depends(y, target)
