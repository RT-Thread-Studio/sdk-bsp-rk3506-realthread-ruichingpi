...
def gen_diff_info(init_cfg, cfg, cic):
    ...
    if os.path.exists(path):
        ...
        for exclude_list in exclude_lists:
            if exclude_list['config_name'] == cfg:
                tmp = []
                for exclude_file in exclude_list['exclude_files']:
                    tmp.append('${BSP_ROOT}/%s' % exclude_file)
                exclude_list['exclude_files'] = tmp

        for type, value in suffixes.items():
            a = diff[type]['add']
            b = diff[type]['del']
            c = [x for x in exclude_list['exclude_files'] if str(x).endswith(value)]

            b.extend([x for x in c if x not in b])
            a = [x for x in a if x not in c]

            diff[type]['add'] = list(set(a))
            diff[type]['del'] = list(set(b))
    ...
    for type, value in suffixes.items():
        ...
        b.extend([x for x in a if x in c and x not in b])
        a = [x for x in a if x not in c]
        a.extend([x for x in b if x in d and x not in a])
        b = [x for x in b if x not in d]
