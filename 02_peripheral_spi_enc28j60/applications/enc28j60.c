...
void enc28j60_isr(void *args)
{
    rt_eth_device_ready(&enc28j60_dev.parent);
...
//            spi_write_op(spi_device, ENC28J60_BIT_FIELD_CLR, EIR, EIR_LINKIF);
            ...
            rt_eth_device_linkchange(&(enc28j60->parent), link_status);
        }
...
rt_err_t enc28j60_attach(const char *spi_device_name)
{
    ...
    if (rt_eth_device_init(&(enc28j60_dev.parent), "e2") == RT_EOK)
    {
        rt_kprintf("[enc28j60] netdev e2 register successfully\n");
    }
    else
    {
        rt_kprintf("[enc28j60] netdev e2 register failed\n");
        return -RT_ERROR;
    }
